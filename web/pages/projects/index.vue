...
import { ProjectsInType } from '~/types'
...
const { data: projectsPagination, pending: loading } = useGetProjects(
  { key: 'projects' },
  projectsIn
)
const itemsLength = computed(() => {
  if (projectsPagination.value) {
    return projectsPagination.value.totalCount
  }
  return 0
})
const projects = computed(() => {
  if (projectsPagination.value) {
    return projectsPagination.value.data
  }
})
</script>
