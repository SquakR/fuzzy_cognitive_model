...
pub fn set_project_user_permissions(
    ...
    let project_user = project_services::find_project_user(connection, project_id, user_id)?;
    let last_status =
        project_services::find_last_status_by_project_user(connection, project_user.id)?;
